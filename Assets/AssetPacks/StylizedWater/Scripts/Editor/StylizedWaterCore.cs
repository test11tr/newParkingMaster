...
namespace StylizedWaterShader
{
    public class StylizedWaterCore : Editor
    {
        ...
        public const string INSTALLED_VERSION = "2.1.8";
            ...
            static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
            {
                foreach (string str in importedAssets)
                {
                    if (str.Contains("StylizedWaterCore.cs"))
                    {
                        ...
                        if (!compatibleVersion || alphaVersion)
                        {
    ...
    public class SWS_Window : EditorWindow
    {
        ...
        void DrawTabs()
        {
            ...
            Texture2D dot = null;
            if (StylizedWaterCore.compatibleVersion && !StylizedWaterCore.alphaVersion) dot = SmallGreenDot;
            else if (StylizedWaterCore.alphaVersion || !StylizedWaterCore.IS_UPDATED) dot = SmallOrangeDot;
        ...
        void DrawInstallation()
        {
            ...
            if (StylizedWaterCore.compatibleVersion == false && StylizedWaterCore.alphaVersion == false)
            {
            ...
            if (StylizedWaterCore.compatibleVersion && !StylizedWaterCore.alphaVersion)
            {
                GUI.contentColor = Color.green;
                EditorGUILayout.LabelField("Compatible");
                GUI.contentColor = defaultColor;
            }
            else if (StylizedWaterCore.alphaVersion)
            {
                GUI.contentColor = new Color(1f, 0.65f, 0f);
            ...
            EditorGUILayout.EndHorizontal();
            if (StylizedWaterCore.alphaVersion)
            {
                EditorGUILayout.HelpBox("Only release Unity versions are subject to support and fixes. You may run into issues at own risk.", MessageType.Warning);
                EditorGUILayout.Space();
            }
            ...
            gles20 = PlayerSettings.GetGraphicsAPIs(EditorUserBuildSettings.activeBuildTarget)[0] == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2;
            #endif
            ...
            {
                GUI.contentColor = Color.green;
                EditorGUILayout.LabelField(PlayerSettings.GetGraphicsAPIs(EditorUserBuildSettings.activeBuildTarget)[0].ToString());
                GUI.contentColor = defaultColor;
            }
            EditorGUILayout.EndHorizontal();
        }
